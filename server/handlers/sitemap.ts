 ...
 */
async function getSitemapUrls(): Promise<SitemapItem[]> {
      ...
      for (const item of section.items) {
        ...
        if (item.items && Array.isArray(item.items)) {
          ...
          if (item.path) {
            ...
            parentNode = sectionNode.children!.find(
  ...
  function flattenNode(node: SitemapNode) {
    ...
    if (node.path === "/") {
      if (sitemapData.home?.lastModified) {
        lastModified = sitemapData.home.lastModified;
      }
    } else if (node.path === "/get-started") {
      if (sitemapData.getstarted?.lastModified) {
        lastModified = sitemapData.getstarted.lastModified;
      }
    } else {
...
export async function handleSitemapRequest(
  ...
  try {
    ...
    try {
      sitemapItems = await getSitemapUrls();
    } catch (error) {
