...
import { withGestures } from "mtrl-addons";
...
const createGestureCard = (config: GestureCardConfig = {}) => {
   ...
   */
  const flipCard = () => {
    const cardInner = component.element.querySelector(
      `.${prefix}-gesture-card-inner`
    );
    if (!cardInner) return;

    state.isFlipped = !state.isFlipped;
    cardInner.classList.toggle(
      `${prefix}-gesture-card-inner--flipped`,
      state.isFlipped
    );
   ...
   */
  const applyTilt = () => {
    component.element.style.transform = `perspective(1000px) rotateX(${state.tiltX}deg) rotateY(${state.tiltY}deg) scale(${state.scale})`;
  };
  ...
  const dismissCard = (direction: string) => {
    ...
    component.element.classList.add(
      `${prefix}-gesture-card--dismiss-${direction}`
    );
